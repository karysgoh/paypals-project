...
export default function Dashboard() {
  ...
  const calculateBalances = useCallback((transactions, userId) => {
    ...
    transactions.forEach((transaction) => {
      ...
      const userIsCreator = transaction.created_by === userId;
      const totalAmount = parseFloat(transaction.total_amount || '0');

      if (userIsCreator && members.length > 0) {
        const amountPerMember = totalAmount / (members.length + 1);
        members.forEach((member) => {
          if (member.user_id !== userId && (!member.payment_status || member.payment_status === 'pending')) {
            owedTo += member.amount_owed ? parseFloat(member.amount_owed) : amountPerMember;
          }
        });
      }

      if (transaction.user_payment_status === 'pending' && transaction.is_user_participant) {
        owes += transaction.user_amount_owed ? parseFloat(transaction.user_amount_owed) : (totalAmount / (members.length + 1));
      }
    });
